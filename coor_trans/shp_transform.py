...
def transform_shpfile_batch(read_path_str, write_path_str, coord_original, coord_target):
    if not os.path.exists(write_path_str):
        os.makedirs(write_path_str)

    for in_file in os.listdir(read_path_str):
        if ".shp" in in_file:
            shpFile = os.path.join(read_path_str, in_file)
            out_path = os.path.join(write_path_str, shpFile.split('\\')[-1])
            transform_shpfile(shpFile, out_path, coord_original, coord_target)
